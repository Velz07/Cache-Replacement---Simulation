...
//LRU Status
void lru_status(struct cache_blk **mem)
{
//print LRU status
cout<<"\n**LRU Stats**";
...
//LRU_update on hit
void lru_update_hit(int col_index,struct cache_blk **mem)
{
...
int lru_replace(struct cache_blk **mem)
{
...
//RAND replace
int rand_replace(struct cache_blk **mem)
{
...
//		fprintf(pFile,"write miss");
    ...
    if(repl == 'L')
    {
    	...
    	blk_replace = lru_replace(mem);
    	lru_update_hit(blk_replace,mem);
    }
    else
    	blk_replace = rand_replace(mem);//replace random logic
